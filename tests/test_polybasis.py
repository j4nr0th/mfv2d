...
def test_addition_unequal1():
    ...
    test_array1 = np.array([0.2, -23.0, 2.41, 63, 2.10, 11.201, 2,])
    test_array2 = np.array([0.3, 0.0, 1, 3.6])
    poly1 = Polynomial1D(test_array1)
    poly2 = Polynomial1D(test_array2)
    combined = poly1 + poly2
...
def test_addition_unequal2():
    ...
    test_array2 = np.array([0.2, -23.0, 2.41, 63, 2.10, 11.201, 2,])
    poly1 = Polynomial1D(test_array1)
    poly2 = Polynomial1D(test_array2)
    combined = poly1 + poly2
...
def test_derivative():
    ...
    assert np.all(derivative.coefficients == (test_array * np.arange(test_array.shape[0]))[1:])
...
def test_antiderivative():
    ...
    assert np.all(antiderivative.coefficients == np.pad(test_array / (np.arange(test_array.shape[0]) + 1), (1, 0)))

def test_multiplication():
    """Check if multiplication of two polynomials gives same results as product of two separate."""
    poly1 = Polynomial1D([0, 1, 41, 20, 100, 40])
    poly2 = Polynomial1D([-1, 2, -3.2, 10., 11])
    prod1 = poly1 * poly2
    prod2 = poly2 * poly1
