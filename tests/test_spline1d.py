...
def test_interpolation(n: int, m: int, evaln: int):
    ...
    for power in range(m):
        ...
        frac = (
            2
            * (eval_nodes - test_nodes[iright - 1])
            / (test_nodes[iright] - test_nodes[iright - 1])
            - 1
        )
        expected = frac**power
        assert (
...
def test_antiderivative(n: int, m: int, ntest: int):
    ...
    spl = Spline1D(
        nodes,
        tuple(
            p.offset_by((nodes[i + 1] + nodes[i]) / 2)
            .scale_by(2 / (nodes[i + 1] - nodes[i]))
            .coefficients
            for i in range(m - 1)
        ),
    )
...
# @pytest.mark.parametrize("n,order", ((2, 1), (4, 3), (10, 5)))
