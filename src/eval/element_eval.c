...
static eval_result_t matrix_as_full(error_stack_t *error_stack, const matrix_t *this, const unsigned order,
                                    const form_order_t form, matrix_full_t *p_out, const allocator_callbacks *allocator)
{
...
typedef eval_result_t (*const bytecode_operation)(
                                    ...
                                    const matrix_full_t *initial)
{
    ...
    eval_result_t res;
    {
        ...
        case MASS_2:
            ...
            break;
        default:
            EVAL_ERROR(error_stack, EVAL_BAD_ENUM, "Mass type %u is not valid.", (unsigned)t);
            return EVAL_BAD_ENUM;
        }
        if (res != EVAL_SUCCESS)
        {
            EVAL_ERROR(error_stack, res, "Failed computing mass matrix.");
            return res;
        }
        if (inverse)
        {
            ...
            if (res != EVAL_SUCCESS)
            {
                EVAL_ERROR(error_stack, res, "Failed inverting mass matrix.");
            }
...
static eval_result_t operation_sum(const void *operations[static MATOP_COUNT], error_stack_t *error_stack,
                                   const unsigned order, const unsigned remaining,
                                   const bytecode_t code[static remaining], const fem_space_2d_t *fem_space,
                                   const field_information_t *vector_fields, const unsigned n_stack, unsigned stack_pos,
                                         ...
                                         const matrix_full_t *initial)
{
    ...
    if (res != EVAL_SUCCESS)
    {
        EVAL_ERROR(error_stack, res, "Could not compute the mixed matrix.");
        return res;
    }
...
MFV2D_INTERNAL
eval_result_t evaluate_block(error_stack_t *error_stack, const form_order_t form, const unsigned order,
                             ...
                             matrix_full_t *p_out, const matrix_full_t *initial)
{
    ...
    const eval_result_t res = execute_next(operations, error_stack, order, n_ops, code, fem_space, vector_fields,
