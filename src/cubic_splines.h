...
*
* The function computes derivatives $\frac{d x}{d t} = k$, such that Hermite polynomials can be used
* to interpolate the polynomial between the nodes $x(i) = x[i]$. As such, an interpolation between
* `x[i]` and `x[i + 1]` can be found as $x(t) = x_i * H_1(t) + x_{i+1} * H_2(t) + k_i * H_3(t) + k_{i+1} * H_4(t)$,
* with $0 \leq t \leq 1$ where $H_1(t)$, $H_2(t)$, $H_3(t)$, and $H_4(t)$ are the Hermite polynomials defined as:
*
* - $H_1(t) = 2 t^3 - 3 t^2 + 1$
* - $H_2(t) = 3 t^2 - 2 t^3$
* - $H_3(t) = t^3 - 2 t^2 + t$
...
* @param work Work array of the same size as `x` and `k`, used for intermediate results.
* @param bc_left Boundary condition to apply on the left boundary of the spline.
* @param bc_right Boundary condition to apply on the right boundary of the spline.
*
* @returns `INTERP_SUCCESS` if successful, `INTERP_ERROR_BAD_SYSTEM` if boundary conditions lead to a system that can't
* be solved easily.
*/
INTERPLIB_INTERNAL
