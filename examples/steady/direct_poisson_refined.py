    ...
    for ip, pval in enumerate(p_vals):
        ...
        solution, stats = solve_system_2d(
            ...
            refinement_settings=RefinementSettings(
                2,
                lambda _e, _: True,  # division_function=divide_new
            ),
        ...
        sol.point_data["u_err2"] = np.linalg.norm(
            sol.point_data["u"] - u_exact(sol.points[:, 0], sol.points[:, 1]), axis=-1
        )

        total_error = sol.integrate_data()
        total_u = total_error.point_data["u_err2"]
        e_vals[ip] = total_u[0]
