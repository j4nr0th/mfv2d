...
def test_string_length_limit():
    ...
    with pytest.raises(ValueError):
        _ElementFormSpecification((long_name, UnknownFormOrder.FORM_ORDER_0))
...
def test_form_order_errors():
    ...
    for val in values:
        if val in UnknownFormOrder:
            _ElementFormSpecification(("test", val))
        else:
            with pytest.raises(ValueError):
                _ElementFormSpecification(("test", val))
...
def test_type_errors():
    ...
    with pytest.raises(TypeError):
        _ElementFormSpecification((123, 5))  # name not a string
    with pytest.raises(ValueError):
        _ElementFormSpecification(("name", "not-an-int"))  # order not int
    with pytest.raises(TypeError):
        _ElementFormSpecification("single string, not tuple")


def test_empty_construction():
    """Check an empty constructor does not work."""
    with pytest.raises(TypeError):
        _ElementFormSpecification()
...
def test_contains():
    ...
    efs = _ElementFormSpecification(*specs)
