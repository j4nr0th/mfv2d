...
class CompositeQMatix:
    ...
    children: tuple[tuple[slice, QMatrix], ...]
    n: int
    own: QMatrix | None

    def __init__(self, own: QMatrix | None = None, *children: QMatrix) -> None:
        self.own = own
        offset = 0
        cv: list[tuple[slice, QMatrix]] = list()
        for c in children:
            end = offset + c.n
    ...
    def __matmul__(
        ...
        if isinstance(other, SparseVector):
            ...
            r = SparseVector.concatenate(*(c @ other[s] for s, c in self.children))
